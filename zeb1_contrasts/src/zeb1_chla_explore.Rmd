...
CHLA10_T1 <- subset(CHLA10, idents = "TGFB1")
CHLA10_veh <- subset(CHLA10, idents = "vehicle")
...
```{r}
# dA peaks/genes for tgfb1 treated chla10
DefaultAssay(CHLA10_T1) <- "ATAC"
Idents(CHLA10_T1) <- 'zeb1_expr'
tgfb1_zeb1_peaks <- FindMarkers(
  object = CHLA10_T1, ident.1 = "ZEB1+", ident.2 = "ZEB1-", test.use = 'wilcox', min.pct = 0.1)
allpeaks_t1_zeb1_genes <- ClosestFeature(CHLA10_T1, regions = rownames(tgfb1_zeb1_peaks))
...
genes_t1_zeb1 <- ClosestFeature(CHLA10_T1, regions = sig_tgfb1_zeb1_peaks)

tgfb1_zeb1_motif_accessibility <- FindMotifs(CHLA10_T1, features = sig_tgfb1_zeb1_peaks)

print(glue(length(sig_tgfb1_zeb1_peaks), " 'significant' peaks for tgfb1 CHLA10 ZEB1 contrast"))
# write.csv(tgfb1_zeb1_peaks, "../results/CHLA10_T1_ZEB1_dA.csv")
```

```{r}
# dA peaks/genes for vehicle treated chla10
DefaultAssay(CHLA10_veh) <- "ATAC"
Idents(CHLA10_veh) <- 'zeb1_expr'
veh_zeb1_peaks <- FindMarkers(
  object = CHLA10_veh, ident.1 = "ZEB1+", ident.2 = "ZEB1-", test.use = 'wilcox', min.pct = 0.1)
allpeaks_veh_zeb1_genes <- ClosestFeature(CHLA10_veh, regions = rownames(veh_zeb1_peaks))
...
genes_veh_zeb1 <- ClosestFeature(CHLA10_veh, regions = sig_veh_zeb1_peaks)

veh_zeb1_motif_accessibility <- FindMotifs(CHLA10_veh, features = sig_veh_zeb1_peaks)

print(glue(length(sig_veh_zeb1_peaks), " 'significant' peaks for vehicle CHLA10 ZEB1 contrast"))
# write.csv(veh_zeb1_peaks, "../results/CHLA10_veh_ZEB1_dA.csv")
```
...
```{r}
# DE for tgfb1 treated chla10
DefaultAssay(CHLA10_T1) <- "RNA"
de_t1 <- FindMarkers(
  object = CHLA10_T1, ident.1 = "ZEB1+", ident.2 = "ZEB1-", test.use = 'wilcox', min.pct = 0.1)
de_t1 <- de_t1[-1, ]
sig_de_t1 <- rownames(de_t1[de_t1$p_val_adj < 0.05, ])

# DE for vehicle treated chla10
DefaultAssay(CHLA10_veh) <- "RNA"
de_veh <- FindMarkers(
  object = CHLA10_veh, ident.1 = "ZEB1+", ident.2 = "ZEB1-", test.use = 'wilcox', min.pct = 0.1)
de_veh <- de_veh[-1, ]
...
```{r}
EnhancedVolcano(de_t1, x = 'avg_log2FC', y = 'p_val_adj', lab = rownames(de_t1), FCcutoff = 0.25, title = "DE genes tgfb1 treated chla10")

EnhancedVolcano(de_veh, x = 'avg_log2FC', y = 'p_val_adj', lab = rownames(de_veh), FCcutoff = 0.25, title = "DE genes vehicle treated chla10")
...
Idents(CHLA10) <- "condition"
condition_peaks <- FindMarkers(CHLA10, ident.1 = "TGFB1", ident.2 = "vehicle", test.use = 'wilcox', min.pct = 0.1)
allpeaks_condition <- ClosestFeature(CHLA10, regions = rownames(condition_peaks))
condition_peaks$closest_gene <- allpeaks_condition$gene_name

print(glue(nrow(condition_peaks[condition_peaks$p_val < 0.0001, ]), " 'significant' peaks for CHLA10 treatment contrast"))
# write.csv(condition_peaks, "../results/CHLA10_treatments_dA.csv")
